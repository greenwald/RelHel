...
const rpwa::primeNumbers::entryType& rpwa::primeNumbers::primeNumber(const size_t& index)
{
    ...
    if (index >= initialSize) {
        if (not _cacheTree) {
            throw std::runtime_error("trying to get prime number from uninitialized object.");
        }
        ...
        if (entriesToRead >= (size_t)_cacheTree->GetEntries()) {
            if (index >= (size_t)_cacheTree->GetEntries()) {
                throw std::runtime_error("prime number with index " + std::to_string(index) + " is not in cache with size "
                                         + std::to_string(_cacheTree->GetEntries()) + ".");
            }
        ...
        for (size_t i = initialSize; i < _primeNumberCache.size(); ++i) {
            if (_cacheTree->GetEntry(i) <= 0) {
                throw std::runtime_error("could not get entry " + std::to_string(i) + " from cache tree.");
            }
