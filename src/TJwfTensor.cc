...
long TTensorTerm::LJContraction(const long& nCon, const bool& even)
{
    ...
    if (not even) {
        if (rOme + rEps + rChi + rPhi != 3) {
            throw std::runtime_error("TTensorTerm::LJContraction: Contraction ended with wrong number of indices!!");
        } else { // eq. (5.21) - (5.23)
            long os = -100;
            if (rOme == 1) {
                os = _ome_pzm[0];
    ...
    if (error) {
        throw std::runtime_error("TTensorTerm::LJContraction: Invalid espilon-contraction occurred");
    }
    shrinkVectors(rOme, rEps, rChi, rPhi);
    return 1;
}
                           ...
                           const TFracNum& prefac)
{
    ...
    if (error) {
        throw std::runtime_error("TTensorTerm::Multiply: Each type can be multiplied only once!");
    }
}
...
void TTensorTerm::AddTwoTerms(const TTensorTerm& other)
{
    if (not SameStructure(other)) {
        throw std::runtime_error("NO NO NO these terms cannot be added!");
    }
                                ...
                                const size_t& rPhi)
{
    if (rOme > _ome_pzm.size() or
            ...
            rPhi > _phi_pzm.size()) {
        throw std::runtime_error("could not shrink arrays, they are already too small.");
    }
    _ome_pzm.resize(rOme);
    _eps_pzm.resize(rEps);
    _chi_pzm.resize(rChi);
